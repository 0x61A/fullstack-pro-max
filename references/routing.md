...
| `design` | "design the UI", "make this look unique", "landing page design", "make it look like <site>", "what style fits this brand", "use a 21st.dev/shadcn theme", "find me a pre-built component" | UI/UX & Distinctive Frontend | Module-backed — always load `references/frontend-distinctiveness.md` for brand-forward work (marketing/landing/portfolio pages); `ui-ux-quickref.md` for color/type/UX fundamentals; `scripts/ui-ux/generate.py` to scaffold tokens/skeletons from a chosen palette+pairing+style; example site URL(s) supplied → `references/reference-site-analysis.md` + `scripts/ui-ux/scan.py` (`--brief` drafts the Reference Design Brief); style/sector named but no URL → `data/ui-ux/known-sites-library.csv` (90 rows, real named sites + evergreen gallery-search source per style) via `scripts/common/search.py --tag style:UXnnn`, then the same reference-site-analysis flow; ready-made component/theme wanted instead of from-scratch → `data/ui-ux/component-libraries.csv` (24 rows: 21st.dev, shadcn/ui, Aceternity UI, Magic UI, Origin UI, Tremor, and more, live-fetch not memorized) via `scripts/common/search.py`, then `references/component-library-integration.md` for the ecosystem-specific copy-paste/CLI/npm-install steps; identity from a verbal brief → `references/art-direction-derivation.md` (five personality axes → Design DNA) + `data/ui-ux/style-vocabulary.csv` |
