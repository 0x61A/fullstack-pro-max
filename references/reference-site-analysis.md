...
3. **Cheap offline first pass when no live browser tool is available.** Run `python3 scripts/ui-ux/scan.py <url>` — stdlib-only, fetches the HTML plus same-origin linked stylesheets and regex-extracts a color-frequency table, `font-family` declarations, and a handful of layout keyword hits (`grid`, `sticky`, `clip-path`, etc.). It does **not** render JavaScript or measure real layout — treat its output as hints to verify, not ground truth. Say so explicitly when using it.
